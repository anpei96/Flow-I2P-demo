...
class Diff2D3D(nn.Module):
    def __init__(self, cfg):
        ...
        # self.img_backbone_an = ImageBackbone(
        #     3,
        #     cfg.model.image_backbone.output_dim,
        #     cfg.model.image_backbone.init_dim,
        #     dilation=cfg.model.image_backbone.dilation,
    ...
    def forward(self, data_dict):
        ...
        '''
        note-0408
            we add an auxiliary task, co-vision region detection (cov-det)
            mask is used as gt labels, it does not used in model input
        '''
        # mask_2d = image[:,3:4,:,:]
        # mask_3d = pcd_feats[:,3:4]
        # image   = image[:,:3,:,:]
        # pcd_feats = pcd_feats[:,:3]
        # output_dict["cov_2d_gt"] = mask_2d
        # output_dict["cov_3d_gt"] = mask_3d

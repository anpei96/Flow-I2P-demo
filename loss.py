...
class OverallLoss_Cof(nn.Module):
    def __init__(self, cfg):
        super(OverallLoss_Cof, self).__init__()
        self.c_loss = CoarseMatchingLoss(cfg)
        self.f_loss = FineMatchingLoss(cfg)
        self.weight_c_loss = cfg.loss.coarse_loss.weight
        self.weight_f_loss = cfg.loss.fine_loss.weight
    ...
    def forward(self, data_dict, output_dict):
        ...
        '''
            add 2d overlap loss
        '''
        y_gt = data_dict["mask_2d_gt"].unsqueeze(0).unsqueeze(0).squeeze(-1)
        y_pd = torch.sigmoid(output_dict["mask_2d_pd"])
        cov_loss_2d = 0.5*self.cls_loss(y_gt, y_pd)
        loss += cov_loss_2d

        '''
            add 3d overlap loss
        '''
        z_gt = data_dict["mask_3d_gt"].reshape((-1,1))
        z_pd = torch.sigmoid(output_dict["mask_3d_pd"])
        cov_loss_3d = 0.5*self.cls_loss(z_gt, z_pd)
        loss += cov_loss_3d
        ...
        if is_need_vis_2d:
            import numpy as np
            import cv2   as cv
            m_2d_gt = y_gt.detach().cpu().numpy()
            m_2d_pd = y_pd.detach().cpu().numpy()
            m_2d_gt = (m_2d_gt*255).astype(np.uint8)[0]
            m_2d_pd = (m_2d_pd*255).astype(np.uint8)[0]
            m_2d_gt = m_2d_gt.transpose((1,2,0))
            m_2d_pd = m_2d_pd.transpose((1,2,0))
            m_2d_gt_vis = cv.applyColorMap(m_2d_gt, cv.COLORMAP_JET)
            m_2d_pd_vis = cv.applyColorMap(m_2d_pd, cv.COLORMAP_JET)
            cv.imshow("m_2d_gt_vis", m_2d_gt_vis)
            cv.imshow("m_2d_pd_vis", m_2d_pd_vis)
            cv.waitKey(0)
            print("min--max-2d: ", np.min(m_2d_pd), np.max(m_2d_pd))
        ...
        if is_need_vis_3d:
            ...
            num = pts.shape[0]
            lab_pd = z_pd.detach().cpu().numpy()
            cls_pd = np.zeros((num, 3))
            cls_pd[:,0:1] = lab_pd[:,0:1]
            cls_pd[:,2] = 1 
            
            pts_gt = pts*1.0
            pts_gt[:,0] -= 5
            lab_gt = z_gt.detach().cpu().numpy()
            cls_gt = np.zeros((num, 3))
            cls_gt[:,0:1] = lab_gt[:,0:1] 
            cls_gt[:,2] = 1 

            pts_vis = np.concatenate((pts,    pts_gt), axis=0)
            cls_vis = np.concatenate((cls_pd, cls_gt), axis=0)
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(pts_vis)
            pcd.colors = o3d.utility.Vector3dVector(cls_vis)
            print("min--max-3d: ", np.min(lab_pd), np.max(lab_pd))
            show_pcd(pcd)

        return {
            "loss": loss, "c_loss": c_loss, "f_loss": f_loss, 
            "f_recall": f_recall, 
            "cov_loss_2d": cov_loss_2d, "cov_loss_3d": cov_loss_3d}
